...
from model4 import Encoder,Seq2Seq
...
with_att = True #決定是否使用attention mechanism ,選擇所用的modelel

pretrained = True #是否調用pretrained vector (如果選true 會在model的embedding調用ChineseFastText model) ,目前只有中文能用
...
class Train():
    ...
    def train_epoch(self,data_loader):
        ...
        for  i,(data) in enumerate(data_loader):
            input ,length, label = data#輸入的資料(文字換成index),句子長度，label(標點的index)
            if  self.use_cuda:
                input = input.cuda()
                label = label.cuda()
                length = length.cuda()
            ...
            #print(label)
            outputs = self.model(input,length)#將資料輸入model(調用model的forward),outputs為評估結果
    ...
    def train(self):
        for fold, (train_idx,val_idx) in enumerate(self.splits.split(np.arange(len(self.dataset)))):
            ...
            if with_att:
                path = '/home/yunghuan/Desktop/PuncTBRNN/model/final_attn.pth.tar'
            else:
                path = '/home/yunghuan/Desktop/PuncTBRNN/model/final_noattn.pth.tar'
...
def main():
    ...
    dataset = PuncDataset(data_path,vocab_path,punc_path)#指定所用的dataset為PuncDataset
    ...
    '''
    if with_att:
        encoder = Encoder(input_size,embedding_dim,hidden_dim,4,4,0.,pretrained)
        #decoder = Decoder(hidden_dim,4,4,0.0)
        model = Seq2Seq(encoder,hidden_dim,4,5)
        save_path = '/home/yunghuan/Desktop/PuncTBRNN/model/final_attn.pth.tar4'
    else:
        save_path = '/home/yunghuan/Desktop/PuncTBRNN/model/final_noattn.pth.tar'
        model = bi_LSTM(input_size,embedding_dim,hidden_dim,2,4,pretrained)
    use_cuda = True
    if use_cuda:
        model = model.cuda()
    print(model)
    ...
    optimizer = torch.optim.Adam(model.parameters(),lr = 9e-4,weight_decay=0.0)#決定optimizer （更新weight 的方式）
    scheduler = ExponentialLR(optimizer, gamma=0.5,verbose = True) # weight decay的方式（非必要）
