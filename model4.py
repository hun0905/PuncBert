...
class Encoder(nn.Module):
    ...
    def __init__(self,input_size,embedding_size,hidden_size,output_size,num_layers,p,pre_trained):
        ...
        self.rnns_f = nn.ModuleList()
        self.rnns_b = nn.ModuleList()
        ...
        for i in range(self.num_layers):
            self.rnns_f.append( nn.GRU(self.embedding_size,self.embedding_size,1) )
            self.rnns_b.append( nn.GRU(self.embedding_size,self.embedding_size,1) )
        self.fc_hidden = nn.Linear(self.embedding_size*2,self.hidden_size*2) #because it is bidirection, so *2.
    ...
    def forward(self,x,length):
        ...
        pack_input_f= pack_padded_sequence(embedding,length.cpu(),batch_first=True)
        pack_input_b= pack_padded_sequence(torch.flip(embedding,[0]),length.cpu(),batch_first=True)
        ...
        hidden_f = torch.zeros((self.num_layers,batch_size,seq_len,self.embedding_size))
        hidden_b = torch.zeros((self.num_layers,batch_size,seq_len,self.embedding_size))
        length = x.size(1)
        for i in range(self.num_layers):
            if i == 0:
                output_f,_ = self.rnns_f[0](pack_input_f)
                output_b,_ = self.rnns_b[0](pack_input_b)
            else:
                output_f,_ = self.rnns_f[i](output_f)
                output_b,_ = self.rnns_b[i](output_b)
            hidden_f[i],_= pad_packed_sequence(output_f,total_length=length,batch_first=True)
            hidden_b[i],_= pad_packed_sequence(output_b,total_length=length,batch_first=True)
        hidden = torch.cat((hidden_f,torch.flip(hidden_b,[0])),3).cuda()
...
class Seq2Seq(nn.Module):
    def __init__(self,encoder,hidden_size,num_layers,heads):
        ...
        self.heads = heads
        self.num_layers = num_layers
        self.predict = nn.Linear(hidden_size*2*(num_layers+1),4)
        self.rnn = nn.GRU(self.hidden_size*2,self.hidden_size*2,batch_first = True)
        self.multi_head_attns = nn.ModuleList()
        for i in range(num_layers):
            self.multi_head_attns.append( nn.MultiheadAttention(self.hidden_size*2,self.heads,batch_first=True,dropout=0.5) )
    ...
    def forward(self,source,length):
        ...
        rnn_output,_ = self.rnn(context[-1]) # output : (seq_len , batch_size , hidden_size)
        for i in range(self.num_layers):
            attn_output[i],_ = self.multi_head_attns[i](rnn_output,context[i],context[i])
        attn = attn_output[0]
        
        for i in range(1,self.num_layers):
            attn = torch.cat((attn,attn_output[i]),2)
        attn = torch.cat((attn,rnn_output),2)
        outputs = self.predict(attn)
        #print(outputs)
