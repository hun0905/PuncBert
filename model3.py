...
class Decoder(nn.Module):
    ...
    def __init__(self,hidden_size,output_size,num_layers,p):
        ...
        self.wf_f = nn.Linear(self.hidden_size,self.hidden_size)
    ...
    def forward(self,x_t,projected_context,context,state = None):
        ...
        fw = torch.sigmoid( self.wf_f(lfc)+ self.wf_h(x_t) ) #fw : (batch_size,hidden_size)
        
        hf_t = lfc*fw+x_t #hf_t : (batch_size,hidden_size)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self,source,length,teacher_force_ratio=0.5):
        ...
        for t in range(1,target_len):
            ...
            #output[t,:,:]:  (batch_size,hidden_size)
            
            #projected_context : (seq_len,batch_size,hidden_size*2)
            output = self.decoder(rnn_output[t,:,:],projected_context,context,rnn_output[t-1,:,:])
